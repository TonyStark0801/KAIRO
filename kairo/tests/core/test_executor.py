...
@pytest.mark.asyncio
async def test_executor_skips_memory_when_disabled(bus, mock_registry, mock_adapter):
    mem_events = []
    bus.subscribe(MemoryWriteEvent, lambda e: mem_events.append(e))
    await asyncio.sleep(0.05)
    executor = ToolExecutor(bus, mock_registry, mock_adapter, timeout=30)
    event = IntentRoutedEvent(
        tool_name="test_tool", params={}, confidence=0.9, session_id="s1",
