...
class ToolExecutor:
    ...
    async def on_intent_routed(self, event: IntentRoutedEvent) -> None:
        ...
        if tool is None:
            await self._publish_result(event.tool_name, ToolResult(success=False, message=f"Tool not found: {event.tool_name}"), event.session_id, event.params)
            return
        try:
    ...
    async def _publish_result(self, tool_name: str, result: ToolResult, session_id: str, params: dict) -> None:
