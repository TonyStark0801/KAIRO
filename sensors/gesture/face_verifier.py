...
class FaceVerifier:
    def __init__(
        ...
        embedding_path: str = "~/.kairo/face_embedding.npy",
    ...
    def initialize(self) -> bool:
        if not self._embedding_path.exists():
            logger.error("No enrolled face at %s — run 'kairo-enroll-face'", self._embedding_path)
            return False
        try:
...
def enroll_cli() -> None:
    """One-time face enrollment — run as 'kairo-enroll-face'."""
    import cv2
    ...
    dest = Path("~/.kairo/face_embedding.npy").expanduser()
    ...
    while len(embeddings) < captures_needed:
        ...
        cv2.imshow("Kairo Enrollment", display)
